...
import { DduSetSymbol, type DduInternalOptions } from "../src/core/types.js";
import {
  Ddu64AdapterError,
  Ddu64ChecksumError,
  Ddu64DecodeError,
...
// Direct-core 동기/난독화 테스트에 필요한 구현을 명시적으로 주입합니다.
...
describe("Ddu64Core", () => {
  ...
  describe("Checksum", () => {
    it("should add and verify checksum", () => {
      ...
      const encoded = encoder.encode(input);
      expect(encoded).toMatch(/CK[PO][0-9a-f]{8}/); // marker + scope + 8 hex
      const decoded = encoder.decode(encoded);
      expect(decoded).toBe(input);
    });
