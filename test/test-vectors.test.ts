 ...
 * against the Ddu64Secure encoder. Vectors tagged "round-trip-only"
 * verify encode→decode round-trip instead of exact encoded output.
 */
import { describe, it, expect } from "vitest";
// 압축·암호화·체크섬 벡터는 동기 adapter가 포함된 secure 진입점으로 검증합니다.
import { Ddu64Secure } from "../src/Ddu64Secure.js";
import { DduSetSymbol } from "../src/core/types.js";
import type { DduConstructorOptions } from "../src/core/types.js";
...
function createEncoder(vector: TestVector): Ddu64Secure {
  ...
  if (dduSetSymbol) {
    return new Ddu64Secure(undefined, undefined, constructorOptions);
  }

  return new Ddu64Secure(dduChar, paddingChar, constructorOptions);
}
