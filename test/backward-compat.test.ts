...
// 압축·체크섬 하위호환 벡터는 동기 adapter가 포함된 secure 진입점으로 검증합니다.
...
describe("구버전 호환성 테스트", () => {
  ...
  describe("기본 인코딩/디코딩", () => {
    it("기본 DDU 프리셋 - 안녕하세요12 (커스텀 charset)", () => {
      ...
      const encoder = new Ddu64Secure(koreanChars, "뭐", { throwOnError: false });
    ...
    it("4문자 charset (우따야야) - usePowerOfTwo", () => {
      ...
      const encoder = new Ddu64Secure("우따야야", "뭐", {
        usePowerOfTwo: true,
        throwOnError: false,
      });
  ...
  describe("프리셋 호환성", () => {
    ...
    it("DDU_V1 프리셋 - 6비트 쌍 인코딩 프로필 유지", () => {
      const encoder = new Ddu64Secure({ dduSetSymbol: DduSetSymbol.DDU_V1 });
  ...
  // ─── 크로스 플랫폼 호환 벡터 ────────────────────────────────────────
  // test/fixtures/compat-vectors.json
  // 63개 엣지 케이스 (다국어, 이모지, 제어문자, 패딩 경계 등)이 들어있습니다.

  describe("크로스 플랫폼 호환 벡터", () => {
    const v2Encoder = new Ddu64Secure();
    const v1Encoder = new Ddu64Secure({ dduSetSymbol: DduSetSymbol.DDU_V1 });
  ...
  describe("고급 기능 호환성", () => {
    ...
    it("커스텀 종성결합 charset - 인코딩/디코딩 호환", () => {
      const encoder = new Ddu64Secure(["가", "나", "다", "라", "마", "바", "사", "아"], "뭐", {
  ...
  describe("생성자 오버로드 호환성", () => {
    it("new Ddu64({ options }) 형태가 new Ddu64(undefined, undefined, { options })와 동일", () => {
      const oldStyle = new Ddu64Secure(undefined, undefined, {
        dduSetSymbol: DduSetSymbol.DDU_V1,
      });
      const newStyle = new Ddu64Secure({ dduSetSymbol: DduSetSymbol.DDU_V1 });
    ...
    it("new Ddu64({ options }) - DDU 기본 프리셋과 동일", () => {
      const defaultEncoder = new Ddu64Secure();
      const optionsEncoder = new Ddu64Secure({ dduSetSymbol: DduSetSymbol.DDU });
