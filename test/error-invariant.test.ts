 ...
 * plain `Error`를 누출하지 않는다. 내부 도메인 에러와 공개 API 경계 래핑이
 * 함께 이 계약을 유지한다.
 */
...
describe("공개 API 에러 불변식 (plain Error 누출 0)", () => {
  const baseEncoders = () => [new Ddu64Node(), new Ddu64Node({ obfuscate: true })];
  ...
  it("decode/decodeToUint8Array: 임의 문자열 입력에 타입 에러만", () => {
    fc.assert(
      fc.property(fc.string(), arbOptions, (input, options) => {
        for (const enc of [...baseEncoders(), ...secureEncoders()]) {
  ...
  it("decodeToBuffer(Node 기본 경로): 임의 문자열 입력에 타입 에러만", () => {
    fc.assert(
  ...
  it("getStats/getStatsAsync: 통계 경로도 타입 에러만 노출한다", async () => {
  ...
  it("사용자 progress callback 오류도 통계 API 경계에서 타입화한다", async () => {
    const enc = new Ddu64Node();
    const options = {
      onProgress: () => {
    ...
    assertTypedOnThrow(() => enc.getStats("stats", options));
    await assertTypedOnReject(() => enc.getStatsAsync("stats", options));
  });
