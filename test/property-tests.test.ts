...
import { parseFooter } from "../src/core/wireFormat.js";
...
describe("Property-Based Tests", () => {
  ...
  describe("Buffer/Uint8Array 동등성", () => {
  ...
  describe("난독화 출력 유효성", () => {
    it("난독화된 출력의 모든 문자가 U+AC00–U+D7A3 범위", () => {
      const encoder = new Ddu64Secure(undefined, undefined, {
  ...
  describe("난독화 라운드트립", () => {
    ...
    it("Ddu64Secure 난독화 인코딩 전체 라운드트립", () => {
      const encoder = new Ddu64Secure(undefined, undefined, {
  ...
  describe("async 경로 동등성", () => {
    it("plain 비동기 출력이 동기 출력과 일치하고 라운드트립", async () => {
      const encoder = new Ddu64Node();

      await fc.assert(
        fc.asyncProperty(fc.uint8Array({ minLength: 1, maxLength: 2000 }), async (data) => {
          const syncEncoded = encoder.encode(data);
          const encoded = await encoder.encodeAsync(data);
          expect(encoded).toBe(syncEncoded);
    ...
    it("압축과 체크섬을 사용한 비동기 출력이 동기 출력과 일치", async () => {
      const encoder = new Ddu64Secure({ compress: true, checksum: true });

      await fc.assert(
        fc.asyncProperty(fc.uint8Array({ minLength: 1, maxLength: 1000 }), async (data) => {
          expect(await encoder.encodeAsync(data)).toBe(encoder.encode(data));
        }),
        { numRuns: 50 },
      );
    });
  ...
  describe("Tree-shaking 검증", () => {
    it("core.ts 엔트리포인트가 Node.js 내장 모듈을 import하지 않음", () => {
      // core.ts 소스 파일 읽기
      const coreSource = fs.readFileSync(path.resolve(__dirname, "../src/core.ts"), "utf-8");
    ...
    it("browser.ts 엔트리포인트가 Node.js 내장 모듈을 import하지 않음", () => {
      const browserSource = fs.readFileSync(path.resolve(__dirname, "../src/browser.ts"), "utf-8");
  ...
  describe("키 없는 난독화 라운드트립 (디코드 키 무관)", () => {
    it("암호화 키 없이 obfuscate한 결과를 디코더 키 보유/미보유와 무관하게 원본 복원", () => {
      // 인코더와 키 없는 디코더는 기본 진입점을 사용합니다.
  ...
  describe("Secure 압축/암호/체크섬 라운드트립", () => {
