/** BrowserAdapter Web API implementation tests. */

import { describe, it, expect, vi } from "vitest";
import { BrowserAdapter } from "../src/adapters/BrowserAdapter.js";
import type { PlatformAdapter } from "../src/core/types.js";
...
describe("BrowserAdapter", () => {
  ...
  describe("deflate / inflate", () => {
    it("지원 API가 나중에 주입되면 capability를 다시 탐지한다", async () => {
      const NativeCompressionStream = globalThis.CompressionStream;
      const original = new TextEncoder().encode("late compression capability");

      try {
        vi.stubGlobal("CompressionStream", undefined);
        await expect(new BrowserAdapter().deflate(original)).rejects.toThrow(/unsupported/i);

        vi.stubGlobal("CompressionStream", NativeCompressionStream);
        await expect(new BrowserAdapter().deflate(original)).resolves.toBeInstanceOf(Uint8Array);
      } finally {
        vi.stubGlobal("CompressionStream", NativeCompressionStream);
        vi.unstubAllGlobals();
      }
    ...
    it("압축과 압축 해제 기능을 독립적으로 탐지한다", async () => {
      const original = new TextEncoder().encode("independent stream capability ".repeat(20));
      const NativeCompressionStream = globalThis.CompressionStream;
      const NativeDecompressionStream = globalThis.DecompressionStream;

      try {
        vi.stubGlobal("DecompressionStream", undefined);
        const compressed = await browserAdapter.deflate(original);

        vi.stubGlobal("DecompressionStream", NativeDecompressionStream);
        vi.stubGlobal("CompressionStream", undefined);
        await expect(browserAdapter.inflate(compressed)).resolves.toEqual(original);
      } finally {
        vi.stubGlobal("CompressionStream", NativeCompressionStream);
        vi.stubGlobal("DecompressionStream", NativeDecompressionStream);
        vi.unstubAllGlobals();
