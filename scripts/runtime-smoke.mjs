 ...
 * 검증합니다. 특히 선언된 최소 Node 버전(>=18)에서 Web Streams를 포함한
...
import { Ddu64, createReadableEncodeStream, createReadableDecodeStream } from "../dist/index.js";
...
async function main() {
  ...
  // 3) 압축 + 체크섬 (zlib + maxOutputLength 한도 경로)
  {
  ...
  // 4) brotli
  {
    const ddu = new Ddu64({ compress: true, compressionAlgorithm: "brotli" });
    const input = "y".repeat(4096);
    eq(ddu.decode(ddu.encode(input)), input, "brotli round-trip");
  }

  // 5) AES-256-GCM 암호화 (pbkdf2 키 파생)
  {
    const ddu = new Ddu64({ encryptionKey: "smoke-secret", checksum: true });
    const input = "보호 대상 데이터";
    eq(ddu.decode(ddu.encode(input)), input, "encrypt round-trip");
  }

  // 6) Web Streams (Node 18 글로벌 TransformStream 게이트)
  {
    const ddu = new Ddu64();
