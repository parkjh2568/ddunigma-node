 ...
 * 한글 음절 난독화 레이어 구현.
 *
 * charset 인코딩 문자열을 결정론적·가역적으로 한글 음절 블록으로 변환합니다.
 * 매핑은 다음을 보장합니다:
 * - 모든 출력 문자가 유효한 한국어 음절 블록 (U+AC00–U+D7A3)
 * - 출력 길이가 입력과 동일한 1:1 매핑
 * 입력 문자 빈도는 보존되므로 보안적 무작위성이나 기밀성을 제공하지 않습니다.
 *
 ...
 * 위치에 따라 각 문자의 할당된 음절 범위를 순환하며 원본 문자로 복원할 수 있습니다.
 */
export class HangulObfuscationLayer implements ObfuscationLayer {
  ...
  /**
   * charset 인코딩 문자열을 한글 음절로 변환합니다.
   *
   * 각 입력 문자는 할당된 범위의 한글 음절에 매핑됩니다.
   * 문자열 내 위치가 범위 내 어떤 음절을 사용할지 결정합니다.
   *
   * @param input - charset 인코딩 문자열
