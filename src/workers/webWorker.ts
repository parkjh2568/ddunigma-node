/**
 * ddunigma 인코딩/디코딩 연산을 위한 Web Worker 진입점.
 *
 * 이 파일은 브라우저 Web Worker 내부에서 실행됩니다.
 * 인코딩/디코딩 요청 메시지를 수신하고 결과를 다시 전송합니다.
 * nodeWorker.ts와 동일한 메시지 프로토콜을 사용합니다.
 *
 * @module workers/webWorker
 */
...
function decodeInWorker(input: string, config: EncoderConfig): Uint8Array {
  ...
  for (let i = 0; i < cleanedInput.length; i++) {
    ...
    if (val === undefined) {
      throw new Error(`[webWorker decode] Invalid character "${char}" at position ${i}`);
    }
    indices[idx++] = val;
  }
  indices.length = idx;
