export class Ddu64 {
    ...
    encode(input: Buffer, option: string = "default"): string {
        ...
        const selectedPadding = option === "KR" ? this.paddingCharKr : this.paddingChar;
        ...
        for (const char of encoded) {
            ...
            result += selectedDduSet[Math.floor(charInt / selectedDduLength)] + selectedDduSet[charInt % selectedDduLength];
        }
    ...
    decode(input: string, option: string = "default"): Buffer {
        ...
        // 패딩 문자 제거
        const paddingCount = (input.match(new RegExp(selectedPadding, 'g')) || []).length;
