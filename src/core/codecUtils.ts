 ...
 *
 * ⚠️ 보안 주의: CRC32는 **우연한 손상(전송/저장 오류) 감지용**이며 변조 방지 수단이 아닙니다.
 * 체크섬 마커는 AES-GCM 인증 영역(AAD) 밖에 부착되므로 공격자가 임의로 재계산할 수 있습니다.
 * 변조 방지가 필요하면 `encryptionKey`(AES-256-GCM)를 사용하세요. GCM authTag가 무결성을
 * 암호학적으로 보장합니다. 비교에 사용하는 `constantTimeEquals`도 비밀값 비교가 아니므로
 * 타이밍 보호 목적이 아니라 단순 일관성용입니다.
 *
