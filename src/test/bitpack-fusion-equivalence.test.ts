...
describe("BitPack fused ↔ reference equivalence", () => {
  ...
  // Deterministic edge-length coverage across the full charset matrix. Random
  // property runs cover the space statistically; these pin the padding-boundary
  // branches (6-bit unroll remainders 1/2 → paddingBits 4/2, 8-bit no-pad,
  // 4-bit generic, non-pow2 pair padding) explicitly so the pre-optimization
  // baseline is fixed at every alignment.
  ...
  // L2 unroll fusion explicit oracle (Task 3.4). The 6/8-bit branches of
  // packPow2ToString / unpackPow2FromString replace the generic accumulator
  // with a direct 3-byte→4-symbol / 1-byte→1-symbol mapping. These assertions
  // explicitly pin that the unrolled branch is byte-identical to the raw
  // reference (`bitPackEncode` → bitPackEncode6/bitPackEncode8 → indices, and
  // `bitPackDecode`), and nail every last-group padding boundary by exact
  // paddingBits value so intent is unambiguous.
  // Requirements: 7.1 (Fusion_Oracle vs raw bitPack), 7.3 (Pow2_Charset),
  // 7.4 (mismatch signals failure via these expect() assertions).
  describe("L2 unroll branch ↔ raw bitPackEncode6/8 + bitPackDecode (explicit)", () => {
    ...
    /** Deterministic byte pattern of the requested length. */
    ...
    /** Assert fused pow2 path is byte-identical to raw reference, with exact paddingBits. */
    function assertUnrollEquivalence(
      ...
      // Encode: fused unroll branch vs raw bitPackEncode6/8 → charCode mapping.
      const ref = bitPackEncode(data, config);
      const fused = packPow2ToString(data, config.bitLength, charCodes);
      ...
      // Decode: fused unroll branch vs raw bitPackDecode over reference indices.
...
// Error-signal preservation edge tests (Task 6.2). The L2/L3 unroll + symmetry
// optimizations must NOT change the decode error signals: same message text and
// same throwing position (index) as the pre-optimization fused path. We assert
// the exact `[Ddu64 decode] ...` messages for the three documented failure
// modes (Design "Error Handling"): charset-foreign character, truncated symbol
// pair (non-pow2), and out-of-range value (non-pow2). Where the raw
// `bitPackDecode` reference exercises the same condition we also pin behavioral
// parity (both reject; identical "... at position i" suffix for range checks).
// Requirements: 4.3
describe("decode error-signal preservation (fused paths)", () => {
  /** A code unit guaranteed to be outside any U+AC00-based charset. */
  const FOREIGN_CHAR = "A"; // U+0041, far below the U+AC00 lookup offset

  /** Replace the character at `pos` in `s` with `ch`. */
  ...
  describe("non-power-of-two charset (50 chars, 6-bit)", () => {
    ...
    /** Encode a small fixed input into a valid non-pow2 payload to corrupt. */
